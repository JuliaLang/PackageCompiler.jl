...
## TODO: Take various --compile=all or --compile=no options
...
function compile(julia_program_file, julia_install_path,
    ...
    COMPILE_MODE = "--compile=all"
    run(`"$(Base.julia_cmd())" "-J$(SYS_LIB)" $(COMPILE_MODE) "--startup-file=no" "--output-o" "$(O_FILE)" "-e" "

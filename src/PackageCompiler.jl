...
function gather_stdlibs_project(ctx)
    ...
    return filter(entry -> last(entry).name in sysimage_stdlibs, collect(ctx.env.manifest))
end
...
function create_sysimage(packages::Union{Nothing, Symbol, Vector{String}, Vector{Symbol}}=nothing;
    ...
    if Sys.isapple()
        cmd = `install_name_tool -id @rpath/$(basename(sysimg_file)) $sysimg_path`
        run(cmd)
    end
...
function create_sysimg_from_object_file(object_files::Vector{String},
    ...
    @show object_files
    @show sysimage_path
    sysimage_dir, sysimage_file = dirname(sysimage_path), basename(sysimage_path)
    cd(sysimage_dir) do
        ...
        cmd = `$(bitflag()) $(march()) -shared -L$(julia_libdir()) -L$(julia_private_libdir()) -o $sysimage_file $o_file_flags -ljulia-internal -ljulia $extra`
...
function create_executable_from_sysimg(exe_path::String,
    ...
    exe_dir, exe_name = dirname(exe_path), basename(exe_path)
    mkpath(exe_dir)
    @show exe_dir
    @show sysimage_path
    cd(exe_dir) do
        sysimage_path = relpath(sysimage_path, exe_dir)
        flags = Base.shell_split(join((cflags(), ldflags(), ldlibs()), " "))
        m = something(march(), ``)
        @show rpath_executable()
        cmd = `-DJULIAC_PROGRAM_LIBNAME=$(repr(sysimage_path)) $TLS_SYNTAX $(bitflag()) $m -o $(exe_name) $(c_driver_program) $(sysimage_path) -O2 $(rpath_executable()) $flags`
        @show cmd
        run_compiler(cmd)
    end
    return nothing
...
function create_library(package_dir::String,
                        ...
                        version::Union{String,VersionNumber}=nothing,
                        compat_level::String="major",
                        cpu_target::String=default_app_cpu_target(),
                        include_lazy_artifacts::Bool=false,
                        sysimage_build_args::Cmd=``,
    ...
    lib_path = Sys.iswindows() ? joinpath(dest_dir, "bin") : joinpath(dest_dir, "lib")

    sysimg_file = get_sysimg_file(name; version)
    compat_file = get_sysimg_file(name; version, compat_level)
    soname = (!Sys.isunix() || Sys.isapple()) ? nothing : compat_file

    create_sysimage_workaround(ctx, package_dir, lib_path, precompile_execution_file,
        precompile_statements_file, incremental, filter_stdlibs, cpu_target;
        julia_init_c_file, version,
        sysimage_build_args, include_transitive_dependencies, soname)

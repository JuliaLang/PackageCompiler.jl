...
function build_object(
  ...
  Sys.__init__(); Base.early_init(); # `JULIA_HOME` is not defined, initialize manually
  include(\"$juliaprog\") # include Julia program file
...
int exit_jl_runtime()
{
    ...
    command = init_shared ?
        `$cc -shared -DJULIAC_PROGRAM_LIBNAME=\"$s_file\" -o $s_file $o_file $i_file $(julia_flags(optimize, debug, cc_flags))` :
